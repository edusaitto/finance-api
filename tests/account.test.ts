...
test("GET to /balance?account_id=1234 should return 404", async () => {
  const response = await fetch(
    "http://127.0.0.1:3001/balance?account_id=1234",
  );
  expect(response.status).toBe(404);
});

test("POST to /event should return 201 for existing account", async () => {
  const firstDeposit = await fetch(
    "http://127.0.0.1:3001/event",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  const secondDeposit = await fetch(
    "http://127.0.0.1:3001/event",
    {
      method: "POST",
      headers: {
...
test("GET to /balance?account_id=100 should return 200", async () => {
  const response = await fetch(
    "http://127.0.0.1:3001/balance?account_id=100",
  );
  expect(response.status).toBe(200);
});
